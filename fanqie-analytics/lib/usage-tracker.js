...
// Buffers counts in memory, flushes to disk every 30s and on exit.
...
// In-memory buffer: "YYYY-MM-DD" → tenantId → { total, endpoints: { "METHOD /path": count } }
const buffer = {};
...
function usageTracker(req, res, next) {
  // Only track requests that have a tenant (set by authMiddleware)
  res.on("finish", () => {
    ...
    // Normalize: strip tenant-specific segments, trailing slashes, query strings
    const normalized = (method + " " + (route || req.path)).replace(/\/+$/, "").slice(0, 120);

    if (!buffer[date]) buffer[date] = {};
    if (!buffer[date][tenant.id]) buffer[date][tenant.id] = { total: 0, endpoints: {} };

    buffer[date][tenant.id].total++;
    buffer[date][tenant.id].endpoints[normalized] =
      (buffer[date][tenant.id].endpoints[normalized] || 0) + 1;
  });
...
function getUsage(tenantId, date) {
  ...
  // Merge in-memory buffer data (not yet flushed)
...
function getMonthlyUsage(tenantId) {
  // Merge all daily files in the current month
  const now = new Date();
...
function getTodayCollectionCount(tenantId) {
  const usage = getTodayUsage(tenantId);
  return usage.endpoints["POST /api/v1/collect"] || 0;
}

module.exports = { usageTracker, getUsage, getTodayUsage, getMonthlyUsage, getAllTenantsUsage, getTodayCollectionCount, flush };
